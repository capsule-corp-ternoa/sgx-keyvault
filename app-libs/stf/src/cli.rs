		...
		.add_cmd(
			Command::new("store-nft-secret")
				...
				.runner(move |_args: &str, matches: &ArgMatches<'_>| {
					...
					let (mrenclave, shard) = get_identifiers(matches);
					let nonce = get_layer_two_nonce!(account, matches);
					let top: TrustedOperation = TrustedCall::store_secret(
						account.public().into(),
						arg_nft_id,
						arg_secret.into(),
					)
					.sign(&KeyPair::Sr25519(account), nonce, &mrenclave, &shard)
		...
		.add_cmd(
			Command::new("retrieve-nft-secret")
				...
				.runner(move |_args: &str, matches: &ArgMatches<'_>| {
					...
					let account = get_pair_from_str(matches, arg_account);
					let top: TrustedOperation =
						TrustedGetter::retrieve_secret(account.public().into(), arg_nft_id)
							.sign(&KeyPair::Sr25519(account))
							.into();
					let res = perform_operation(matches, &top);
					print!("received response for retrieve-nft-secret: ");
