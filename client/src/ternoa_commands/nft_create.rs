...
/// INPUT: AccountId (owner), ASCII encoded URI to fetch additional metadata.
pub fn nft_create_cli_command() -> Command<'static, str> {
    Command::new("nft-create")
...
fn add_arguments<'a, 'b>(app: App<'a, 'b>) -> App<'a, 'b> {
    ...
    .arg(
        Arg::with_name("details")
            ...
            .required(true)
            .value_name("STRING")
            .help("ASCII encoded URI to fetch additional metadata"),
    )
}
...
fn command_runner<'a>(
    ...
    let details: &str = matches.value_of("details").unwrap();
    debug!("entering nft create function, owner: {}, details: {}", owner_ss58, details);
