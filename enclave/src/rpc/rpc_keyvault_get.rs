...
use substratee_stf::{Getter, ShamirShare, TrustedGetter, TrustedOperation};
...
impl RpcGet {
    ...
    fn method_impl(
        &self,
        request: Request,
    ) -> Result<(Option<ShamirShare>, bool, DirectRequestStatus), String> {
        debug!("entering keyvault_get RPC");
        ...
        let nft_id = match trusted_getter_signed.getter {
            TrustedGetter::keyvault_get(_, c) => Ok(c),
            _ => Err(RpcCallStatus::operation_type_mismatch.to_string()),
        }?;

        let share = match self.rpc_gateway.keyvault_get(main_account, nft_id) {
            Ok(b) => Ok(b),
...
pub mod tests {
    ...
    use crate::rpc::rpc_keyvault_check::RpcCheck;
    use alloc::boxed::Box;
    use sp_core::Pair;
