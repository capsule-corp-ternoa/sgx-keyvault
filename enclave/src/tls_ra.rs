...
fn tls_server_config(sign_type: sgx_quote_sign_type_t, skip_ra: bool) -> SgxResult<ServerConfig> {
    let (key_der, cert_der) = create_ra_report_and_signature(sign_type, skip_ra).sgx_error()?;
...
pub extern "C" fn request_key_provisioning(
    ...
    let cfg = match tls_client_config(sign_type, skip_ra == 1) {
        Ok(cfg) => cfg,
        Err(e) => return e,
    };
...
fn tls_client_config(sign_type: sgx_quote_sign_type_t, skip_ra: bool) -> SgxResult<ClientConfig> {
    let (key_der, cert_der) = create_ra_report_and_signature(sign_type, skip_ra).sgx_error()?;
